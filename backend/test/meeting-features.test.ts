...
test('meeting details reject ungrounded reminders and invalid dates/spans, while retaining real topic chapters',()=>{
  ...
  assert.equal(result.conversations[0]?.action_items.length,2);assert.equal(result.conversations[0]?.action_items[1]?.due_date,null);assert(!result.conversations[0]?.action_items.some(item=>item.task==='Invented'));assert.equal(result.conversations[0]?.chapters?.length,2);assert.equal(result.conversations[0]?.unresolved_questions?.[0]?.text,'Who approves?');
});
