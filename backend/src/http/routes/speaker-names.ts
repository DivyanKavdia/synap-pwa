...
import { indexMemory } from '../../pipeline/index-memory.js';
...
export function speakerNameRoutes(): Router {
  ...
  router.get('/recordings/:recordingId/speakers', requireAuth(), handler<AuthedRequest>(async (req, res) => {
    ...
    res.json({ speakers: transcriptSpeakers(transcript.originalText), speaker_names: readSpeakerNames(req.uid, recording, req.dek), self_label: recording.selfSpeakerLabel || null, supports_self_label: true, names_confirmed:Boolean(recording.sealedSpeakerNames), revision: recording.updatedAt });
  }));
  router.post('/recordings/:recordingId/speakers', requireAuth(), handler<AuthedRequest>(async (req, res) => {
    ...
    const selfLabel = req.body?.self_label === undefined ? recording.selfSpeakerLabel || null : req.body.self_label;
    if (selfLabel !== null && (typeof selfLabel !== 'string' || selfLabel === 'S?' || !Object.hasOwn(names,selfLabel)))
      throw new HttpError(400,'invalid_self','Choose your named speaker, or leave your identity unassigned.');
    const oldNames = readSpeakerNames(req.uid, recording, req.dek);
    const same = JSON.stringify(Object.entries(names).sort()) === JSON.stringify(Object.entries(oldNames).sort()) && selfLabel === (recording.selfSpeakerLabel || null);
    ...
    if (same) memory = openJson<StructuredMemory>(req.dek, recording.sealedMemory, binding(req.uid, `recording/${id}`, 'memory'));
      ...
      memory = await extractMemory({ transcript: applySpeakerNames(source.originalText, names), confirmedSpeakers: names,
        selfSpeakerName: selfLabel ? names[selfLabel] : names.YOU,
        startedAt: recording.startedAt, day: recording.day, timezone: recording.timezone,
    ...
    if(!same || !recording.sealedSpeakerNames) {
      ...
      const fields = {
        ...
        selfSpeakerLabel: selfLabel,
      };
      try { await indexMemory(req.uid, req.dek, { ...recording, ...fields }, memory, '', { expectedRevision: recording.updatedAt, fields }); }
      catch (error) { if ((error as Error).message === 'Recording changed during correction') throw new HttpError(409, 'changed', 'This recording changed while rebuilding. Reload speaker names and try again.'); throw error; }
      revision = (await db.getRecording(req.uid, id))!.updatedAt;
    }
    let dayUpdated = true;
    ...
    res.json({ ...memory, recording_id: id, started_at: recording.startedAt, day: recording.day, duration_ms: recording.durationMs,
      transcript: applySpeakerNames(source.originalText, names), raw_transcript: source.originalText, speaker_names: names, self_label: selfLabel, names_confirmed:true, revision, day_updated: dayUpdated });
  }));
  return router;
}
