...
export function speakerNameRoutes(): Router {
  ...
  router.get('/recordings/:recordingId/speakers', requireAuth(), handler<AuthedRequest>(async (req, res) => {
    ...
    res.json({ speakers: transcriptSpeakers(transcript.originalText), speaker_names: readSpeakerNames(req.uid, recording, req.dek), revision: recording.updatedAt });
  }));
  router.post('/recordings/:recordingId/speakers', requireAuth(), handler<AuthedRequest>(async (req, res) => {
    ...
    const source = await materializeTranscript(req.uid, recording, req.dek);
    ...
    res.json({ ...memory, recording_id: id, started_at: recording.startedAt, day: recording.day, duration_ms: recording.durationMs,
      transcript: applySpeakerNames(source.originalText, names), raw_transcript: source.originalText, speaker_names: names, revision, day_updated: dayUpdated });
  }));
  return router;
}
