# Upload and transcription recovery
...
- `serviceModelCooldowns` stores hashed model names, expiry, quota category, rejection count and last-rejection time. A 429 suppresses calls across instances. `SYNAP_SHARED_MODEL_COOLDOWN` defaults to `1`; unit fixtures disable it and test the store separately. No user data or credentials are stored there.
...
Reload to shell137-cooldown, then retry the affected saved recording. Audio upload can complete while the provider is limited; processing tries again after its cooldown. No re-recording or clearing browser storage is required.
...
## Follow-up — shell137-cooldown

The 05:52–06:00 phone log still ends in a rate-limit failure. Review found that the cloud transport assigned `providerStatus: 429` both to a real Gemini rejection and to work suppressed by an existing shared cooldown. The log therefore did not prove another provider request occurred. The cloud's retry interval also stayed near one minute even when the phone backed off for longer.

- A suppressed request now produces `processing_deferred`, with no provider HTTP status. It cannot increase the browser's rate-limit counter or consume a normal failure attempt.
- Actual provider rejections increase durable model backoff through 1, 2, 4, 8 and 15 minutes, always respecting a longer provider deadline. Concurrent rejections within one active cooldown share a round. Reading a cooldown never increases its count or extends its deadline. An hour without rejections resets the streak.
- Processing diagnostics include the internally configured model, model stage, failure timestamp, and whether the source was the provider or an existing cooldown. No provider response body is exposed.
- Job identifiers distinguish the two different recordings the local scheduler can process concurrently. These identifiers appear in diagnostics, not the normal queue status.

Regression fixtures verify these behaviors, including a retry accepted by the cloud followed by a suppressed model call. These corrections reduce wasted retries and make the remaining failure diagnosable. They do not establish that Gemini's live quota is available or that a production transcript has completed.
