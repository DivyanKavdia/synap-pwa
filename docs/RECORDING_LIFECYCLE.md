...
Every Stop, including a pendant-initiated drain, has one recording-owned watchdog. Reconnect cannot reset its 35-second deadline. Complete frames extend the progress window; status notifications or partial packets alone do not. The no-progress limit is eight seconds during a connected drain, or 20 seconds while restoring the connection. A restored drain gets a fresh progress window within the same absolute deadline. At the deadline, the app disconnects if necessary and seals the original journal as `stop-unconfirmed`. Missing audio is not synthesized as recovered speech. Normal acknowledged drain still saves as `normal`.

Two-byte recorder commands prefer advertised writes without an ATT response; application status remains the acknowledgement. A settled native Stop rejection gets up to three attempts. If those fail on firmware with buffered recovery, the app preserves the journal before disconnecting and resumes its Stop on the new link. It does not seal merely because the write failed. Advertisements respect reconnect backoff and cannot keep resetting the retry counter.

New compacted segments store a contiguous PCM ArrayBuffer in IndexedDB, bounded to 960 kB per 30-second window. The byte write and deletion of represented packets share one transaction; partial packets remain. Existing `pcmBlob` rows remain supported. If whole-Blob readers fail, a legacy read can recover through 64 kB slices only when every slice is complete. Unreadable originals are retained. The upload regression covers 399 complete frames plus one partial frame, reload, exact upload bytes, and legacy Blob reader failures.
