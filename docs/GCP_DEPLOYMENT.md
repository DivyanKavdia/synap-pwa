...
For an **existing production project**, use its original Terraform state and
follow [production adoption](TERRAFORM_ADOPTION.md). Never initialize an empty
state and apply it against the existing project.

## 3. Provision secrets before the first service

Secret versions are provisioned outside Terraform. There is no placeholder key
and no session-key rotation during infrastructure apply. Before a fresh install:

1. Create `synap-gemini-api-key` and `synap-session-signing-key` in Secret Manager.
2. Add the real Gemini API key through a secure local prompt or the cloud console.
   Generate a random 32-byte base64 signing key and store it directly in the
   signing secret. Do not put either value in Terraform variables or chat.
3. Import the two secret metadata resources into Terraform. Verify enabled
   versions exist without printing their payloads.
4. Set `service_url` to the Cloud Run HTTPS service origin used as the Tasks OIDC
   audience. For an existing service, use `status.url`; do not change it during
   adoption. For a new service, Google provides the deterministic
   `https://SERVICE-PROJECT_NUMBER.REGION.run.app` URL.
5. Apply a reviewed plan with `project_id`, `google_client_id`, `image`, and
   `service_url`. Set `operations_invoker_sa` to the existing deployment service
   account. Deploy from CI with the synthetic readiness check before promotion.

The runtime refuses to start with missing queue identity or placeholder secrets.
Neither secret payload belongs in Terraform state for a fresh installation.
Existing state snapshots may contain the old generated signing key and must
remain private even after the non-destructive `removed` blocks detach it.
