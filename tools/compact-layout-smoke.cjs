...
const root = process.env.SYNAP_UI_ROOT || path.resolve(__dirname, '..'),
  out = process.env.SYNAP_UI_OUTPUT || '/tmp/synap-compact-measure';
const baseline = process.env.SYNAP_BASELINE === '1',
  measurements = [];
...
async function fixture(page) {
  await page.evaluate(async () => {
    ...
    const summary =
      'We reviewed the prototype microphone placement and agreed to compare the two enclosure designs. Alex will bring the revised drawings on Friday. The next test should cover a quiet room, a busy café, and walking outdoors. The team will use the same voice samples for every comparison, document the noise floor, and keep the original audio for review.';
    const samples = new Int16Array(16000),
      header = new DataView(new ArrayBuffer(44)),
      ascii = (at, s) => [...s].forEach((c, i) => header.setUint8(at + i, c.charCodeAt(0)));
    ascii(0, 'RIFF');
    header.setUint32(4, 32036, true);
    ascii(8, 'WAVE');
    ascii(12, 'fmt ');
    ...
    ascii(36, 'data');
    header.setUint32(40, 32000, true);
    await new Promise((resolve) => {
      const tx = db.transaction('recordings', 'readwrite');
      for (let i = 0; i < 6; i++) {
        const person = ['Alex', 'Sam', 'Maya', 'Jordan', 'Riya', 'Chris'][i],
        ...
        const conversation = {
          ...
          summary,
          participants: ['You', person],
          start_ms: 0,
          decisions: [{ text: 'Compare both enclosure designs.' }],
          action_items: [{ task: 'Bring revised drawings', owner: person, due_date: '2026-09-11' }],
          topics: ['Prototype', 'Design'],
        ...
        tx.objectStore('recordings').put({
          ...
          summary,
          transcript: 'You: Let’s compare both designs. ' + person + ': I will bring the drawings.',
          blob: new Blob([header.buffer, samples], { type: 'audio/wav' }),
          meeting: {
  ...
  await page.waitForFunction(
    () => document.getElementById('conversationCount').textContent === '6',
  );
  ...
  try {
    for (const width of [390, 1440]) {
      ...
      await context.addInitScript(() => {
        if (!localStorage.getItem('synap-appearance'))
          localStorage.setItem('synap-appearance', 'light');
      });
      ...
      await page.waitForFunction(() => window.SynapBrainUI && window.SynapDashboardUI);
      await fixture(page);
      ...
      if (!baseline) {
        ...
        const actionsToggle = page.locator('#myActions > .tile-heading .tile-toggle');
        ...
        await page.locator('#recording-compact-0 audio').evaluate((node) => {
          window.savedPlayer = node;
        });
        const notes = page.locator('#recording-compact-0 .recording-notes');
        await notes.evaluate((node) => {
          window.savedNotes = node;
          node.closest('details').open = true;
        });
        await notes.fill('An edit that must survive collapse.');
        await page.locator('#library .tile-toggle').click();
        assert(!(await page.locator('#libraryBody').isVisible()));
        await page.locator('.brain-tabs a[href="#library"]').click();
        assert(
          await page
            .locator('#recording-compact-0 audio')
        ...
        assert(
          await memory.evaluate((node) => node.open),
          'memory stays open through data refresh',
        );
        await page.evaluate(() => {
        ...
        while (await page.locator('main>.workspace-tile.is-expanded .tile-toggle').count())
          await page.locator('main>.workspace-tile.is-expanded .tile-toggle').first().click();
        await page.locator('.brain-tabs a[href="#today"]').click();
        ...
        for (const palette of ['olive', 'blue', 'pink', 'lavender'])
          for (const mode of ['light', 'dark']) {
            ...
            assert.equal(await page.evaluate(() => localStorage.getItem('synap-appearance')), mode);
            assert.equal(await page.evaluate(() => localStorage.getItem('synap-palette')), palette);
            ...
            assert(
              await page.evaluate(() => {
                const button = document.querySelector('.synap-account button');
                if (!button) return true;
                const hex = getComputedStyle(document.documentElement)
                    .getPropertyValue('--on-action')
                    .trim(),
                  rgb = hex
                    .slice(1)
                    .match(/../g)
                    .map((c) => parseInt(c, 16));
                return getComputedStyle(button).color === 'rgb(' + rgb.join(', ') + ')';
              }),
              'account action uses the correct contrasting text color',
            );
            const prefix = palette === 'olive' ? '' : palette + '-';
            for (const logo of await page.locator('.synap-brand-image').all()) {
              assert.equal(
                await logo.getAttribute('src'),
                'synap-logo-' + prefix + mode + '.png?v=1.0.0-ui-fix1',
              );
              assert(
            ...
            await page
              ...
              .screenshot({ path: path.join(out, `palette-${palette}-${mode}-${width}.png`) });
            await page.locator('#closeSettingsButton').click();
            await page.evaluate(() => window.scrollTo(0, 0));
            await page.screenshot({ path: path.join(out, `home-${palette}-${mode}-${width}.png`) });
            await page.locator('#settingsButton').click();
          }
        assert.equal(colors.size, 8);
        await page.locator('[data-palette-choice="pink"]').click();
        await page.locator('[data-theme-choice="system"]').click();
        await page.clock.setSystemTime(new Date('2026-09-10T15:00:00Z'));
        await page.evaluate(() => window.dispatchEvent(new Event('focus')));
        assert.equal(await page.locator('html').getAttribute('data-theme'), 'dark');
        assert.equal(await page.locator('html').getAttribute('data-palette'), 'pink');
        await page.reload();
        await page.waitForFunction(() => window.SynapCompactLayout);
        assert.equal(await page.locator('html').getAttribute('data-palette'), 'pink');
        assert.equal(await page.locator('html').getAttribute('data-theme'), 'dark');
        assert.equal(
          await page.locator('#library .tile-toggle').getAttribute('aria-expanded'),
          'false',
      ...
      await context.close();
      console.log('PASS ' + width + 'px; initial populated page ' + height + 'px');
    }
