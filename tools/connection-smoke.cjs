...
(async () => {
  ...
  try {
    ...
    if (process.env.SYNAP_RECOVERY_FIXTURE === '1') {
      await page.goto(origin + '/?buffered');
      ...
      await page.waitForFunction(() => bleFixture.captured >= 12);
      await page.evaluate(() => {
        bleFixture.hide();
        bleFixture.disconnect();
      });
      await page.waitForTimeout(1200);
      const offline = await page.evaluate(() => bleFixture.captured);
      assert(offline >= 30, 'capture continues while disconnected');
      ...
      const stored = await page.evaluate(async () => {
        ...
        return (await store.all('recordings'))[0];
      });
      assert.equal(
        stored.stats.completeFrames,
        captured,
        'every captured offline and live frame is saved',
      );
      assert.equal(stored.stats.missingFrames, 0);
      assert.equal(stored.durationMs, captured * 50);
      assert.equal(
        await page.evaluate(() => bleFixture.starts),
        1,
        'recovery never starts a second firmware recording',
      );
      assert.deepEqual(errors, []);
      console.log(
        'PASS buffered recovery: offline capture, same timeline, no START and stop drains queued audio',
      );
      for (const mode of ['after', 'before', 'expired']) {
        ...
        const previous = await page.evaluate(async () => ({
          ids: (await new DKAudioStore().all('recordings')).map((r) => r.id),
        ...
        await page.waitForFunction(
          (starts) =>
            ...
            bleFixture.starts === starts + 1 &&
            bleFixture.captured >= 12,
          previous.starts,
        );
        await page.evaluate(() => {
          bleFixture.holdReplay(true);
          bleFixture.hide();
          bleFixture.disconnect();
        });
        ...
        await page.evaluate(
          (mode) => bleFixture.interruptNextStop(mode === 'before' ? 'before' : 'after'),
          mode,
        );
        await page.locator('#headerCaptureToggle').click();
        await page.waitForFunction(
        ...
        const stoppedAt = await page.evaluate(() => bleFixture.captured);
        await page.waitForTimeout(250);
        if (mode !== 'before')
          assert.equal(
            await page.evaluate(() => bleFixture.captured),
            stoppedAt,
            'acknowledged Stop keeps the microphone off while disconnected',
          );
        const pending = await page.evaluate(
          async (ids) =>
            (await new DKAudioStore().all('recordings')).find((r) => !ids.includes(r.id)),
          previous.ids,
        );
        assert.equal(
          pending.status,
          'recording',
          'the journal is not sealed while remaining audio can be recovered',
        );
        await page.evaluate((mode) => {
          bleFixture.holdReplay(false);
          if (mode === 'expired') bleFixture.expireBuffer();
          bleFixture.show();
        }, mode);
        await page.waitForFunction(
          () =>
        ...
        if (mode !== 'expired') {
          assert.equal(saved.stats.completeFrames, await page.evaluate(() => bleFixture.captured));
          assert.equal(saved.stats.missingFrames, 0);
        }
        assert.equal(
          await page.evaluate(() => bleFixture.starts),
          previous.starts + 1,
          'reconnecting after Stop must never issue START',
        );
        assert.deepEqual(errors, []);
        console.log(
          'PASS buffered Stop/' +
            mode +
            ': Stop intent survives reconnect and the same journal is saved without another START',
        );
      }
      await page.waitForFunction(() => !document.body.hasAttribute('data-auto-reconnecting'));
      const previous = await page.evaluate(async () => ({
      ...
      await page.evaluate(() => {
        ...
        IDBObjectStore.prototype.put = function (value, ...rest) {
          if (this.name === 'recordings' && value.audioQuality && !qaQualityAborts) {
            qaQualityAborts++;
        ...
        bleFixture.hide();
        bleFixture.disconnect();
        bleFixture.invalidRecoveryOnce();
        bleFixture.show();
      });
      await page.waitForFunction(() =>
        document
          .querySelector('#diagnosticsLog')
          .textContent.includes('recovery information was incomplete'),
      );
      await page.waitForFunction(
        () =>
          document.body.dataset.state === 'recording' &&
      ...
      assert.equal(await page.evaluate(() => qaQualityAborts), 1);
      assert.equal(retained.status, 'saved');
      assert.equal(retained.stats.completeFrames, await page.evaluate(() => bleFixture.captured));
      assert.equal(retained.stats.missingFrames, 0);
      assert.equal(await page.evaluate(() => bleFixture.starts), previous.starts + 1);
      assert.deepEqual(errors, []);
      console.log(
        'PASS incomplete recovery reply retries safely; an optional quality transaction failure cannot prevent sealing audio',
      );
      await context.close();
      return;
    }
      ...
      page.evaluate(
        () =>
          new Promise((resolve, reject) => {
            ...
            r.onsuccess = () => {
              const db = r.result,
                read = db.transaction('recordings').objectStore('recordings').getAll();
              read.onsuccess = () => {
                db.close();
                resolve(
                  read.result.map((x) => ({
                    ...
                    rememberMarkers: x.rememberMarkers,
                  })),
                );
              };
            };
    ...
    await page.locator('#markMoment').click();
    fs.mkdirSync('/tmp/synap-moments-qa', { recursive: true });
    await page.screenshot({ path: '/tmp/synap-moments-qa/recording-light.png' });
    await page.evaluate(() =>
      window.dispatchEvent(
        new StorageEvent('storage', { key: 'synap-appearance', newValue: 'dark' }),
      ),
    );
    await page.screenshot({ path: '/tmp/synap-moments-qa/recording-dark.png' });
    await page.evaluate(() =>
      window.dispatchEvent(
        new StorageEvent('storage', { key: 'synap-appearance', newValue: 'light' }),
      ),
    );
    await page.waitForFunction(
      () => document.querySelector('#momentFeedback').textContent === 'Moment saved',
    );
    const first = await records();
    assert.equal(first[0].rememberMarkers.length, 1);
    assert(first[0].rememberMarkers[0].offsetMs > 0);
    assert.equal(first.length, 1);
    await page.evaluate(() => {
      bleFixture.hideOnNextConnect();
      bleFixture.disconnect();
    });
    ...
    assert.equal(resumed[0].id, first[0].id);
    ...
    assert(
      ...
      'header Stop keeps Settings open',
    );
    await page.locator('#settingsButton').click();
    await page.waitForFunction(() => !document.querySelector('#settingsDialog').open);
    const saved = await records();
    assert.equal(saved[0].rememberMarkers.length, 1, 'bookmark survives reconnect and saving');
    assert(!(await page.locator('#markMoment').isVisible()));
    assert.equal(await page.locator('.brain-tabs a').count(), 4);
    assert.equal(await page.locator('#rememberThis').count(), 0);
    assert(!(await page.locator('#capture').isVisible()));
    assert.equal(
      await page.evaluate(() =>
        SynapMoments.mark().then(
          () => false,
          () => true,
        ),
      ),
      true,
      'idle marks are rejected',
    );
    assert.equal(saved.length, 1);
    assert.equal(saved[0].id, first[0].id);
    assert(saved[0].durationMs >= 1200);
    assert.equal(await page.evaluate(() => window.bleFixture.maximum), 1);
    assert.deepEqual(errors, []);
    await page.locator('.brain-tabs a[href="#library"]').click();
    await page.locator('#recording-' + saved[0].id + ' > summary').click();
    await page.waitForFunction(() => !!document.querySelector('.recording-moments button'));
    assert.equal(await page.locator('.recording-moments button').count(), 1);
    await page.locator('.recording-moments button').click();
    await page.waitForFunction(() => {
      const a = document.querySelector('.recording-content audio');
    ...
    assert.equal(
      ...
      1,
      'reload reuses permission without a chooser',
    );
    assert.equal(
      await page.evaluate(() => bleFixture.starts),
      starts,
      'reload does not start recording',
    );
    assert(
      !(await page.locator('#capture').isVisible()),
      'automatic recovery does not expose redundant controls',
    );
    await sleepPendant();
    await page.waitForFunction(
      () => SynapSleepStateGuard.locked && document.body.dataset.state === 'disconnected',
    );
    ...
    assert.equal(
      ...
      sleepingConnects,
      'no immediate reconnection during sleep transition',
    );
    await page.clock.fastForward(6000);
    await page.waitForFunction(() => bleFixture.watching);
    await page.evaluate(() => bleFixture.wake());
    await page.waitForFunction(() => document.body.dataset.state === 'idle');
    assert.equal(
      await page.evaluate(() => SynapSleepStateGuard.locked),
      false,
      'live service clears stale sleep lock',
    );
    assert.equal(await page.evaluate(() => bleFixture.pickers), 1);
    console.log(
      'PASS: reload restores the permitted pendant and a wake advertisement reconnects without another picker or START',
    );

    await page.evaluate(() => bleFixture.disableAdvertisements());
    await sleepPendant();
    await page.waitForFunction(
      () => SynapSleepStateGuard.locked && document.body.dataset.state === 'disconnected',
    );
    ...
    try {
      ...
      console.error(
        'Periodic wake recovery failed',
        await page.evaluate(() => ({
          state: document.body.dataset.state,
          connects: bleFixture.connects,
          maximum: bleFixture.maximum,
          locked: SynapSleepStateGuard.locked,
    ...
    assert(
      (await page.evaluate(() => bleFixture.connects)) > unavailableAttempts,
      'periodic recovery detects wake without advertisements',
    );

    await sleepPendant();
    await page.waitForFunction(() => SynapSleepStateGuard.locked);
    await page.locator('#settingsButton').click();
    await page.locator('#autoReconnectInput').uncheck();
    ...
    const disabledAt = await page.evaluate(() => {
      bleFixture.wake();
      return Date.now();
    });
    await page.clock.runFor(31000);
    await page.waitForTimeout(100);
    assert(
      await page.evaluate((start) => Date.now() - start >= 31000, disabledAt),
      'the disabled interval must elapse before enabling reconnect',
    );
    assert.equal(
      await page.evaluate(() => bleFixture.connects),
      disabledConnects,
      'off preference is respected during sleep/wake',
    );
    await page.locator('#autoReconnectInput').check();
    await page.waitForFunction(() => document.body.dataset.state === 'idle');
    await page.locator('#closeSettingsButton').click();
    assert.equal(await page.evaluate(() => bleFixture.starts), starts);
    console.log(
      'PASS: periodic fallback recovers a later wake and respects the reconnect preference without changing recording state',
    );
    ...
    assert.equal(
      await page.evaluate(() => bleFixture.pickers),
      1,
      'persisted sleep state cannot block reload recovery',
    );
    assert.equal(await page.evaluate(() => bleFixture.starts), starts);
    console.log(
      'PASS: a stale sleep flag from the previous page clears after reload connects to the awake pendant',
    );

    await page.goto(origin + '/?noRestore=1');
    await page.waitForFunction(
      () =>
        window.SynapCompactLayout &&
        document.querySelector('#diagnosticsLog')?.textContent.includes('Application started'),
    ...
    await page.locator('.brain-tabs a[href="#today"]').click();
    await page.waitForFunction(() =>
      document.querySelector('#reconnectStatus').textContent.includes('tap on Connect'),
    );
    assert.equal(
      await page.evaluate(() => bleFixture.pickers),
      1,
      'unsupported restore never prompts automatically',
    );
    await page.locator('#settingsButton').click();
    ...
    assert.equal(
      await page.evaluate(() => bleFixture.pickers),
      2,
      'manual connection remains usable on limited browsers',
    );
    assert.deepEqual(errors, []);
    console.log(
      'PASS: browsers without getDevices show an actionable hint and retain manual connection',
    );
    ...
    await page.evaluate(() => {
      ...
      sessionStorage.setItem('qa-retain-link', '1');
    });
    await page.goto(origin);
    await page.waitForFunction(
      () =>
    ...
    assert.equal(
      await page.evaluate(() => bleFixture.connects),
      nativeConnects,
      'a retained native link is adopted without connect()',
    );
    assert.equal(
      await page.evaluate(() => bleFixture.appDisconnects),
      0,
      'adopting a retained link never disconnects it',
    );
    await page.waitForFunction(() => document.body.dataset.eventChannel === 'event');
    console.log(
