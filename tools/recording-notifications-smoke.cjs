...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(
      () => document.body.dataset.startup === 'ready' && navigator.serviceWorker.controller,
    );
    ...
    await worker.evaluate(() => {
      ...
      self.registration.showNotification = async (title, options) => {
        const entry = {
          ...
          ...options,
          close() {
            if (entries.get(options.tag) === entry) entries.delete(options.tag);
          },
        };
    ...
    const notifications = () =>
      worker.evaluate(async () =>
        (await self.registration.getNotifications({ tag: 'synap-recording' })).map((n) => ({
          title: n.title,
          data: n.data,
          actions: n.actions,
        })),
    ...
    async function waitForNotifications(count) {
      ...
      while (Date.now() < deadline) {
        if ((await notifications()).length === count) return;
        await new Promise((resolve) => setTimeout(resolve, 50));
      }
      throw Error('Notification count did not become ' + count);
    }
    const act = (action, sessionId) =>
      worker.evaluate(
        async ({ action, sessionId }) => {
          const notification = (
            await self.registration.getNotifications({ tag: 'synap-recording' })
          )[0];
          const owner = await self.clients.get(notification.data.ownerClientId);
          return sendRecordingAction(
            owner,
            { data: { ...notification.data, ...(sessionId ? { sessionId } : {}) } },
            action,
          );
        },
        { action, sessionId },
    ...
    assert.equal(
      await page.locator('#recordingNotificationInput').isEnabled(),
      true,
      JSON.stringify(
        await page.evaluate(() => ({
          capabilities: SynapRecordingNotifications.capabilities(),
          hint: document.getElementById('recordingNotificationHint').textContent,
          errors: document.getElementById('diagnosticsLog').textContent.slice(-400),
        })),
      ),
    );
    ...
    assert.equal(
      ...
      0,
      'enabling notifications cannot pretend recording has begun',
    );
    ...
    await page.waitForFunction(() => SynapAppControls.recordingState().canMark);
    await page.evaluate(() => SynapRecordingNotifications.sync());
    ...
    await page.waitForFunction(() => document.body.dataset.audioDelivery === 'waiting');
    await page.evaluate(() => SynapRecordingNotifications.sync());
    assert.deepEqual((await notifications())[0].actions.map((a) => a.action), ['stop']);
    assert.equal(await act('mark'), false, 'an audio interruption cannot accept a moment marker');
    await page.evaluate(() => bleFixture.blockAudio(false));
    await page.waitForFunction(() => SynapAppControls.recordingState().canMark);
    await page.evaluate(() => SynapRecordingNotifications.sync());
    ...
    assert.equal(
      (await notifications()).length,
      0,
      'off preference leaves recording usable without a notification',
    );
    await page.locator('#headerCaptureToggle').click();
    await page.waitForFunction(() => document.body.dataset.state === 'idle');
    assert.deepEqual(errors, []);
    console.log(
      'PASS: real worker notification, durable marked moment, Stop/save/cleanup, stale-take rejection and opt-out during live BLE capture.',
    );
    await context.close();
  } finally {
    await browser.close();
