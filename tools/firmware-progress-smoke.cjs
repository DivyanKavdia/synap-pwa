...
async function run(){
  ...
  try{for(const [mode,width] of [['light',320],['dark',390],['light',1440]]){
    ...
    await page.goto(origin);await page.waitForFunction(()=>window.SynapSettingsPanel&&document.querySelector('#headerCaptureToggle')&&document.querySelector('#diagnosticsLog')?.textContent.includes('Application started'));
    await page.evaluate(source=>{
      const node=id=>document.getElementById(id);
      // Bind the production handlers once to fresh controls, retaining real layout and navigation.
      for(const id of ['otaLatest','otaCancel','otaReleaseCheck','firmwareUpdateButton']){const button=node(id);button.replaceWith(button.cloneNode(true));}
      const qa=window.otaUiFixture={flashes:0,holdCheck:false};
      const gate=name=>new Promise((resolve,reject)=>{qa[name]={resolve,reject}});
      let connected=true,build=503;
      const id='SYNAP-AABBCCDDEEFF',manifest={build:1001,identity:'SYNAP-FW:fixture:1001'};
      class Client{
        constructor(io){this.io=io;this.committing=false;qa.progress=value=>io.progress('Updating · '+Math.floor(value*100)+'%',value,false)}
        async check(){if(qa.holdCheck){qa.holdCheck=false;await gate('check')}return{protocol:3,deviceId:id,build,state:1,capacity:2048,maxData:503}}
        async update(){qa.flashes++;this.committing=false;this.io.progress('Updating · 0%',0,false);await gate('transfer');this.committing=true;connected=false;this.io.progress('Restarting pendant…',1,true)}
        reset(){}cancel(){}
      }
      ...
      const context={globalThis:{SynapOTA:{Client},SynapReleases:releases,SynapSettingsPanel:window.SynapSettingsPanel},deviceAssociation:{deviceId:id},connectionEpoch:0,
        ...
        openSettings:()=>node('settingsDialog').open?SynapSettingsPanel.close():SynapSettingsPanel.open(),
    ...
    await page.evaluate(()=>otaUiFixture.check.resolve());await page.waitForFunction(()=>!!otaUiFixture.download);
    assert.equal(await page.locator('#otaStatus').textContent(),'Downloading update…');
    await page.waitForFunction(()=>document.querySelector('#headerPendantStatus').textContent==='Updating');
    await page.locator('#settingsButton').click();assert(await page.locator('#firmwareUpdateSpinner').isVisible());
    await page.evaluate(()=>otaUiFixture.download.resolve(new Blob(['fixture'])));await page.waitForFunction(()=>!!otaUiFixture.transfer);
    await page.evaluate(()=>otaUiFixture.progress(.42));
    assert.equal(await page.locator('#firmwareNoticeText').textContent(),'Updating · 42%');
    assert.equal(await page.locator('#firmwareNoticeProgress').evaluate(node=>node.position),.42);
    assert(!(await page.locator('#firmwareUpdateButton').isVisible()));
    await page.screenshot({path:path.join(out,`firmware-${mode}-${width}.png`)});
    await page.locator('#settingsButton').click();assert.equal(await page.locator('#otaStatus').textContent(),'Updating · 42%');
    assert.equal(await page.locator('#otaProgress').evaluate(node=>node.position),.42);assert(!(await page.locator('#otaLatest').isVisible()));
    await page.locator('#settingsButton').click();await page.evaluate(()=>otaUiFixture.transfer.resolve());await page.waitForFunction(()=>!!otaUiFixture.reboot);
    assert.equal(await page.locator('#firmwareNoticeText').textContent(),'Restarting pendant…');assert(await page.locator('#firmwareUpdateSpinner').isVisible());
    assert.equal(await page.locator('#firmwareNoticeProgress').evaluate(node=>node.position),1);
    assert(await page.locator('#otaCancel').isDisabled(),'cannot cancel a committed image');
    await page.evaluate(()=>otaUiFixture.reboot.resolve());await page.waitForFunction(()=>!!otaUiFixture.reconnect);
    assert.equal(await page.locator('#firmwareNoticeText').textContent(),'Reconnecting to verify update…');
    await page.evaluate(()=>otaUiFixture.reconnect.resolve());await page.waitForFunction(()=>document.querySelector('#firmwareNoticeText').textContent==='Update complete · 1001');
    assert(!(await page.locator('#firmwareNoticeProgress').isVisible()));assert(!(await page.locator('#firmwareUpdateSpinner').isVisible()));assert(!(await page.locator('#firmwareUpdateButton').isVisible()));
    for(const cancel of [false,true]){
      ...
      assert.equal(await page.evaluate(()=>otaUiFixture.flashes),1,'failed/canceled downloads never start a flash');
      await page.locator('#settingsButton').click();assert(!(await page.locator('#firmwareUpdateSpinner').isVisible()));assert(!(await page.locator('#firmwareNoticeProgress').isVisible()));assert(await page.locator('#firmwareUpdateButton').isEnabled());
    }
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert.deepEqual(errors,[]);
    console.log(`PASS firmware/${mode}/${width}: Settings start, shared live percentage, navigation, reboot verification, failure and cancellation`);await context.close();
