...
async function seed(page) {
  await page.evaluate(async () => {
    ...
    const recording = {
      ...
      status: 'saved',
      sealed: true,
      blob: DKAudioCodec.wav([new Uint8Array(32000 * 30)]),
      transcript: '[00:01] Asha: Discuss the budget.\n[00:07] You: Review the invoice.',
      summary: 'Budget reviewed',
      meeting: {
        executive_summary: 'Budget reviewed',
        ...
        conversations: [
          {
            ...
            end_ms: 20000,
            people: names.map((name) => ({ name })),
            decisions: [{ text: 'Approve budget', start_ms: 8000 }],
            action_items: actions,
            follow_ups: [{ text: 'Send revised quote', owner: 'Asha', start_ms: 11000 }],
    ...
    const legacy = {
      ...
      status: 'saved',
      sealed: true,
      summary: 'Older discussion with Iris',
      transcript: 'Iris discussed the earlier project',
      meeting: {
        people: [{ name: 'Iris' }],
        executive_summary: 'Older discussion with Iris',
        action_items: [{ task: 'Check old estimate', owner: 'self', start_ms: 3000 }],
      },
    };
    await journal.atomic(['recordings'], (stores) => {
      stores.recordings.put(recording);
      stores.recordings.put(legacy);
    });
    window.qa = {
      signedIn: false,
      uid: 'actions-user',
      ...
      followups: Array.from({ length: 34 }, (_, i) => ({
        id: 'follow-' + i,
        task: 'Cloud follow-up ' + i,
        kind: 'commitment',
        state: 'open',
        owner: { type: i % 2 ? 'other' : 'self', display_name: i % 2 ? 'Asha' : 'You' },
        source: { recording_id: 'action-source', start_ms: 7000 + i * 10 },
      })),
    };
    SynapAuth.isSignedIn = () => qa.signedIn;
    SynapAuth.session = () => ({ profile: { uid: qa.uid } });
    ...
    SynapAuth.authedFetch = async (url, options = {}) => {
      ...
      if (kind === 'preparation')
        return reply({
          history: [
            {
              title: 'Cloud budget recap',
              summary: 'Budget reviewed',
              source: { recording_id: 'action-source', start_ms: 7000 },
              questions: [],
            },
          ],
          open_actions: [
            {
              text: 'Cloud invoice',
              owner: 'self',
              source: { recording_id: 'action-source', start_ms: 9000 },
            },
          ],
          scope: 'Recent related conversations and open actions.',
        });
      if (kind === 'ask')
        return reply({
          answer: 'The budget was reviewed.',
          confidence: 'high',
          sources: [
            { recording_id: 'action-source', start_ms: 7000, quote: 'Discuss the budget.' },
          ],
          searched: { conversations: 1 },
        });
      if (kind === 'source')
        return reply({
          recording_id: 'action-source',
    ...
    qa.refreshSession = async () => {
      ...
      localStorage.setItem(
        SynapAuth.STORAGE_KEY,
        JSON.stringify({ refreshToken: 'fixture-refresh', profile: { uid: qa.uid } }),
      );
  ...
  try {
    for (const [mode, width] of [
      ...
      await page.waitForFunction(
        () => document.body.dataset.startup === 'ready' && window.SynapInteractionSurfaces,
      );
      await seed(page);
      ...
      const prep = page.locator('.meeting-preparation');
      await prep.getByText('Who signs? · 0:13').waitFor();
      await page.waitForFunction(() => document.activeElement?.id === 'meetingPreparationTitle');
      assert(
        await prep.locator('h3').evaluate((node) => {
          const a = node.getBoundingClientRect(),
            b = document.getElementById('myActionsContent').getBoundingClientRect();
          return a.top >= b.top && a.bottom <= b.bottom;
        }),
        'Prepare must bring its heading into the visible Actions area without another scroll',
      );
      assert.match(
        await prep.innerText(),
        /Review item 8/,
        'local preparation includes mentioned actions with honest status',
      );
      ...
      assert.match(
        await prep.innerText(),
        /Older discussion with Iris/,
        'People and Prepare include older, legacy local memories',
      );
      await prep.getByRole('button', { name: 'Close', exact: true }).tap();
      ...
      await page.waitForFunction(
        () =>
          Math.abs(document.querySelector('#recording-action-source audio').currentTime - 9) < 1,
      );
      await page.locator('.brain-tabs a[href="#myActions"]').tap();
      ...
      await page.getByRole('button', { name: 'Retry People', exact: true }).tap();
      await page.locator('#peopleSearch input').fill('');
      await page.waitForFunction(() =>
        document.querySelector('#peopleList [data-person-id="person-0"]'),
      );
      ...
      await nameInput.fill('Asha Rao');
      await page.evaluate(() => SynapPeopleConfirmUI.decorate());
      assert.equal(
        await nameInput.inputValue(),
        'Asha Rao',
        'background decoration preserves an active name editor',
      );
      ...
      assert(
        ...
        'name editor stays inside the mobile viewport',
      );
      await page.evaluate(() => {
        qa.fail = 'person-save';
      });
      await person.getByRole('button', { name: 'Save', exact: true }).tap();
      await person.getByText('Temporary person-save outage').waitFor();
      assert.equal(
        await nameInput.inputValue(),
        'Asha Rao',
        'failed saves retain the entered name',
      );
      ...
      await person.getByRole('button', { name: 'Save', exact: true }).tap();
      await page
        .getByRole('button', { name: 'Prepare for meeting with Asha Rao', exact: true })
        .waitFor();
      await page
        .getByRole('button', { name: 'Prepare for meeting with Asha Rao', exact: true })
        .tap();
      ...
      assert.match(
        await page.locator('#askAnswer').innerText(),
        /Search cancelled/,
        'cancelled requests cannot overwrite the next state',
      );
      ...
      await page.evaluate(() => {
        qa.fail = 'source';
        const source = document.querySelector('#askAnswer .source-jump');
        source.dataset.id = 'cloud-only-source';
        source.dataset.offsetMs = '7000';
      });
      await page.locator('#askAnswer .source-jump').first().tap();
      await page.getByRole('button', { name: 'Retry source', exact: true }).waitFor();
      assert.match(
        await page.locator('.synap-source-status').innerText(),
        /Temporary source outage/,
      );
      await page.evaluate(() => {
        qa.fail = '';
      });
      await page.getByRole('button', { name: 'Retry source', exact: true }).tap();
      await page.waitForFunction(
        () => document.querySelector('#recording-cloud-only-source')?.open,
      );
      assert.match(
        await page.locator('#recording-cloud-only-source .recording-transcript').inputValue(),
        /Discuss the budget/,
      );
      await page.locator('.brain-tabs a[href="#myActions"]').tap();
      await page.locator('#actionsTab-peopleMemory').tap();
      ...
      assert(
        await prep.isVisible(),
        'a token refresh for the same account must not close preparation',
      );
      assert(
        await page.evaluate(
          () => !qa.held.find((item) => item.kind === 'preparation').signal.aborted,
        ),
      );
      await page.evaluate(() => {
        qa.hold = '';
        qa.held.filter((item) => item.kind === 'preparation').forEach((item) => item.resolve());
      });
      await prep.getByText('Cloud invoice · 0:09').waitFor();
      await prep.getByRole('button', { name: 'Close', exact: true }).tap();
      await page.locator('#peopleSearch input').fill('Blair');
      const blair = page.locator('#peopleList .person-entry').first();
      ...
      assert(
        !(await page.locator('#askInput').isEnabled()),
        'same-account token rotation retains the active search',
      );
      await page.evaluate(async () => {
        qa.signedIn = false;
        await SynapAuth.signOut();
        qa.held.filter((item) => item.kind === 'ask').forEach((item) => item.resolve());
      });
      ...
      assert.equal(
        ...
        0,
        'a previous account cannot publish its late answer',
      );
      assert.deepEqual(errors, []);
      console.log(
        `PASS functional Actions/${mode}/${width}: visible preparation, older local evidence, all next steps, independent cloud lists, completion retry/race, name correction/confirmation, bounded Ask with sources and token/account changes`,
      );
      await context.close();
    }
  } finally {
