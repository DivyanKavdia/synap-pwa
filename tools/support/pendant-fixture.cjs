...
module.exports = function pendantFixture(){
   ...
   let firmwareBuild=1200,otaState=1,otaSession=0,otaOffset=0,otaBegins=0,otaCommits=0,otaHold=false;
   const otaStatus=()=>{const v=new DataView(new ArrayBuffer(20));v.setUint8(0,0xd7);v.setUint8(1,3);v.setUint8(2,otaState);v.setUint32(4,otaSession,true);v.setUint32(8,otaOffset,true);v.setUint32(12,0x140000,true);v.setUint16(16,503,true);v.setUint16(18,firmwareBuild,true);return v;};
   ...
   class Characteristic extends EventTarget{
     ...
     readValue(){return operation(async()=>{if(this.id===uuid('49'))return otaStatus();if(this.id===uuid('4b'))return new DataView(new TextEncoder().encode('SYNAP-FW:'+target+':1.0.0:'+firmwareBuild).buffer);if(this.id===uuid('4f')){if(invalidRecoveryReads>0){invalidRecoveryReads--;return new DataView(new ArrayBuffer(4))}return recoveryStatus()}if(this.id===uuid('47')){statusReads++;if(holdRead){holdRead=false;await new Promise(resolve=>{releaseRead=resolve})}return status()}return this.id===uuid('4c')?new DataView(new TextEncoder().encode('SYNAP-ABCDEF123456').buffer):new DataView(new Uint8Array([0xe2,1,1,0,0x82,4]).buffer)})}
     writeValueWithResponse(value){return operation(async()=>{
       if(this.id===uuid('48')){
         const v=new DataView(value.buffer,value.byteOffset,value.byteLength),command=value[0];
         if(command===1){otaBegins++;otaSession=v.getUint32(1,true);otaOffset=0;otaState=3;}
         if(command===2){while(otaHold)await new Promise(resolve=>setTimeout(resolve,10));if(v.getUint32(5,true)!==otaOffset)throw Error('Unexpected firmware offset');otaOffset+=value.length-9;}
         if(command===3)otaState=4;
         if(command===4){otaCommits++;otaState=5;firmwareBuild=1201;setTimeout(()=>{otaState=1;loseLink()},30);}
         if(command===5)otaState=6;
         const ack=chars.get(uuid('49'));ack.value=otaStatus();ack.dispatchEvent(new Event('characteristicvaluechanged'));return;
       }
       if(this.id===uuid('4f')){
