...
async function fixture(page) {
  await page.evaluate(async () => {
    ...
    await journal.atomic(['recordings'], stores => {
      for (const id of ids) {
        ...
        qa.sources[id] = { recording_id: id, started_at: createdAt, duration_ms: 1000,
          state: id === 'partial' ? 'failed' : 'ready', retryable: id === 'partial',
    ...
    SynapAuth.authedFetch = async (url, options = {}) => {
      ...
      if (source) {
        ...
        if (qa.holdSource === id) await new Promise(resolve => { qa.releaseSource = resolve; });
      ...
      if (/^\/v1\/recordings\?/.test(url)) {
        if (qa.holdHistory) await new Promise(resolve => { qa.releaseHistory = resolve; });
        return reply({ recordings: ids.map(id => {
          const { transcript, transcript_complete, transcript_segments, segment_count, ...metadata } = qa.sources[id];
          return metadata;
        }) });
      }
  ...
  await page.waitForFunction(() => document.querySelectorAll('#insightsList .insight-card[data-recording-id]').length === 3);
}
...
async function run() {
  ...
  try {
    for (const [mode, width] of [['light', 320], ['dark', 390]]) {
      ...
      await page.waitForFunction(() => document.body.dataset.startup === 'ready' && window.SynapProvenance && window.SynapExperienceRecovery);
      await fixture(page);

      // Reproduce the real day-list payload: summary metadata, no transcript.
      const preserved = await page.evaluate(async () => {
        await SynapCloudHistory.restoreAndShow(true, { day: document.querySelector('#datePicker').value, transcript: false });
      ...
      assert.deepEqual(preserved, { transcript: 'Preserved spoken words', notes: 'Original note', bytes: 32044 });
      ...
      assert.match(await memory.locator('.synap-provenance-transcript').innerText(), /Loading transcript/);
      await page.evaluate(() => { qa.holdSource = ''; qa.releaseSource(); });
      await page.waitForFunction(() => document.querySelector('#insightsList [data-recording-id="missing"].insight-card .synap-provenance-transcript')?.textContent.includes('Recovered spoken words for missing'));
      assert.equal(await tab.getAttribute('aria-selected'), 'true', 'hydration keeps the selected Transcript tab');
      assert(await memory.locator('.synap-provenance-transcript').isVisible());
      assert.equal(await page.evaluate(() => qa.calls.filter(call => call.url === '/v1/recordings/missing/source').length), 1, 'one shared source read');
      await page.evaluate(async () => {
        await SynapCloudHistory.restoreAndShow(true, { day: document.querySelector('#datePicker').value, transcript: false });
      });
      assert.match(await memory.locator('.synap-provenance-transcript').innerText(), /Recovered spoken words for missing/);
      assert.equal(await tab.getAttribute('aria-selected'), 'true');
      if (process.env.SYNAP_TRANSCRIPT_OUTPUT) {
        fs.mkdirSync(process.env.SYNAP_TRANSCRIPT_OUTPUT, { recursive: true });
        await memory.screenshot({ path: path.join(process.env.SYNAP_TRANSCRIPT_OUTPUT, `memory-${mode}-${width}.png`) });
      }

      // An old request must not roll back text/notes saved while it was in flight.
      await page.evaluate(() => { qa.holdHistory = true; qa.history = SynapCloudHistory.restore(true, { transcript: false }); });
      await page.waitForFunction(() => qa.releaseHistory);
      await page.evaluate(async () => {
        const journal = new DKAudioStore();
        await journal.atomic(['recordings'], stores => {
          const get = stores.recordings.get('cached');
          get.onsuccess = () => stores.recordings.put({ ...get.result, transcript: 'Recovered during refresh', notes: 'Edited during refresh' });
        });
        qa.holdHistory = false; qa.releaseHistory(); await qa.history;
      });
      ...
      await retry.locator('.recording-disclosure > summary').filter({ hasText: /^Transcript/ }).tap();
      await retry.getByRole('button', { name: 'Retry transcript', exact: true }).waitFor({ state: 'visible' });
      assert.match(await retry.locator('.synap-transcript-notice').innerText(), /Could not load the transcript/);
      await page.evaluate(() => { qa.sourceFailure = ''; });
      await retry.getByRole('button', { name: 'Retry transcript', exact: true }).tap();
      await page.waitForFunction(() => document.querySelector('#recording-retry .recording-transcript')?.value.includes('Recovered spoken words for retry'));
      ...
      await partial.locator('.recording-disclosure > summary').filter({ hasText: /^Transcript/ }).tap();
      await page.waitForFunction(() => document.querySelector('#recording-partial .recording-transcript')?.value.includes('Recovered spoken words for partial'));
      assert(await partial.locator('.recording-transcript').isVisible());
      await page.waitForFunction(() => document.querySelector('#recording-partial .synap-transcript-notice')?.textContent.includes('available so far'));
      assert.match(await partial.locator('.synap-transcript-notice').innerText(), /available so far/);
      assert.equal(await page.evaluate(async () => (await new DKAudioStore().get('recordings', 'partial')).processingStage), 'failed');
      await page.evaluate(() => { qa.sources.partial.transcript = ''; });
      await partial.getByRole('button', { name: 'Refresh transcript', exact: true }).tap();
      await page.waitForFunction(() => document.querySelector('#recording-partial .synap-transcript-notice')?.getAttribute('aria-busy') === 'false');
      assert.match(await partial.locator('.recording-transcript').inputValue(), /Recovered spoken words for partial/, 'an empty incomplete response keeps the preserved transcript');
      const calls = await page.evaluate(() => qa.calls.filter(call => call.url.endsWith('/source')).length);
      await page.waitForTimeout(350);
      assert.equal(await page.evaluate(() => qa.calls.filter(call => call.url.endsWith('/source')).length), calls, 'restoring a selected tab does not refetch in a loop');
      assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), 'mobile transcript view fits');
      assert.deepEqual(errors, [], 'no uncaught page errors');
      console.log(`PASS transcripts/${mode}/${width}: metadata preservation, missing-tab recovery, stable selection, concurrent edits, load retry and partial evidence`);
      await context.close();
    }
