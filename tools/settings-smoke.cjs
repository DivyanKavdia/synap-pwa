...
const root = path.resolve(__dirname, '..'),
  out = process.env.SYNAP_SETTINGS_OUTPUT || '/tmp/synap-settings-qa';
    ...
    for (const mode of ['light', 'dark'])
      for (const width of [320, 390, 1440]) {
        ...
        await page.waitForFunction(
          () => window.SynapSettingsPanel && document.querySelector('#headerCaptureToggle'),
        );
        assert.equal(
          await page
            .locator('#openAsk,#synapSearchMemories,#setupConnect,.settings-brand-logo')
            .count(),
          0,
        );
        ...
        const before = await page.evaluate(() => {
          window.originalHeader = document.querySelector('.topbar');
          return { scroll: scrollY, header: originalHeader.getBoundingClientRect().toJSON() };
        });
        ...
        assert(
          await page.evaluate(() => originalHeader === document.querySelector('.topbar')),
          'same header node and handlers',
        );
        assert(
          await page.locator('#settingsDialog').evaluate((node) => !node.matches(':modal')),
          'settings does not make the header inert',
        );
        const after = await page.locator('.topbar').boundingBox();
        for (const key of ['x', 'y', 'width', 'height'])
          assert(Math.abs(after[key] - before.header[key]) < 1, 'header position: ' + key);
        for (const selector of [
          '#headerPendantStatus',
          '#headerCaptureToggle',
          '#headerBatteryStatus',
          '#settingsButton',
          '.brain-tabs a[href="#myActions"]',
        ])
          await hit(page, selector);
        await page.locator('#headerBatteryStatus').click();
        assert(
          await page.locator('#synapBatteryPopover').isVisible(),
          'battery details still open above settings',
        );
        await page.locator('#headerBatteryStatus').click();
        const controls = [
          'setupDeviceId',
        ...
        for (const id of controls)
          assert.equal(await page.locator('[id="' + id + '"]').count(), 1, 'one owner for ' + id);
        assert.equal(
          await page.locator('#connectButton:visible').count(),
          0,
          'header owns the single visible connection control',
        );
        ...
        for (const section of ['memory', 'appearance', 'support', 'device']) {
          ...
          assert(
            ...
            'tab labels fit without clipping or overlapping',
          );
          ...
          if (section === 'memory') {
            ...
            assert(await page.locator('#endpointInput').isVisible());
            await page.locator('#providerInput').selectOption('synap');
            assert(await page.locator('#synapSignInButton').isVisible());
            await page.locator('#settingsProcessingOptions > summary').click();
          }
          if (section === 'appearance') {
            ...
            assert.equal(
              await page.evaluate(() => localStorage.getItem('synap-palette')),
              'lavender',
            );
          ...
          await page.screenshot({
            path: path.join(out, `settings-${section}-${mode}-${width}.png`),
          });
        }
        assert(
          await page.evaluate(
            () =>
              settingsOriginalInput === document.getElementById('wakeLockInput') &&
              !settingsOriginalInput.checked,
          ),
          'panels preserve live inputs and drafts',
        );
        await page.locator('#settingsTab-device').focus();
        await page.keyboard.press('End');
        assert.equal(
          await page.locator('#settingsTab-support').getAttribute('aria-selected'),
          'true',
        );
        ...
        assert(
          ...
          'single tap closes without fallback reopening',
        );
        assert(!(await page.locator('main').evaluate((node) => node.inert)));
        const returnedScroll = await page.evaluate(() => scrollY);
        assert(
          Math.abs(returnedScroll - before.scroll) < 1,
        ...
        assert.equal(
          await page.evaluate(
            () => JSON.parse(localStorage.getItem('dk-pendant-settings')).wakeLock,
          ),
          false,
        );
        await page.locator('#settingsButton').click();
        ...
        console.log(
          `PASS settings/${mode}/${width}: fixed header, battery, independent scroll, navigation, save, Escape and deduplication`,
        );
        await context.close();
      }
  } finally {
