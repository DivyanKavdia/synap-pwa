...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/__enhancement') {
    res
      ...
      .end(
        '<!doctype html><title>Local enhancement check</title><script src="audio-enhancement.js"></script>',
      );
    return;
  }
...
(async () => {
  ...
  const browser = await chromium.launch({
    headless: true,
    executablePath: process.env.SYNAP_CHROMIUM_PATH || undefined,
  ...
  try {
    ...
    const result = await page.evaluate(async (bytes) => {
      const api = window.SynapAudioEnhancement,
        source = new Blob([new Uint8Array(bytes)], { type: 'audio/wav' }),
        progress = [];
      const started = performance.now(),
        copy = await api.enhance(source, { onProgress: (item) => progress.push(item) });
      const result = new Uint8Array(await copy.arrayBuffer()),
      ...
      return {
        ...
        noiseRms: Math.sqrt(noise / 3000),
        voiceRms: Math.sqrt(voice / 17000),
        preserved: before.every((value, i) => value === bytes[i]),
        busy: api.busy(),
        last: progress.at(-1),
    ...
    assert.equal(result.last.stage, 'complete');
    assert.ok(result.voiceRms > result.noiseRms);
    assert.ok(result.voiceRms > 20);
    const cancelled = await page.evaluate(async (bytes) => {
      const controller = new AbortController();
      try {
        await SynapAudioEnhancement.enhance(new Blob([new Uint8Array(bytes)]), {
          signal: controller.signal,
          onProgress: (item) => {
            if (item.stage === 'processing') controller.abort();
          },
        });
        return false;
      } catch (error) {
        return error.name === 'AbortError' && !SynapAudioEnhancement.busy();
      }
    }, bytes);
    ...
    await page.evaluate(async () => {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error('Production service worker installation did not complete.')),
            10000,
          ),
        ),
      ]);
    ...
    assert.equal(offline.size, bytes.length);
    assert.equal(offline.busy, false);
    assert.deepEqual(external, []);
    assert.deepEqual(errors, []);
    console.log(
      JSON.stringify(
        { status: 'passed', online: result, offline, cancelled, externalRequests: external.length },
        null,
        2,
      ),
    );
