...
async function seed(page) {
  await page.evaluate(async () => {
    const db = await new Promise(resolve => { const q = indexedDB.open('dk-pendant-recordings'); q.onsuccess = () => resolve(q.result); });
    const pcm = new Int16Array(16000), header = new DataView(new ArrayBuffer(44));
    ...
    await new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        tx.objectStore('recordings').put({ id: 'journey-' + i, name: 'Prototype ' + i, createdAt, ownerUid: 'journey-user',
          status: 'complete', sealed: true, durationMs: 1000, sampleRate: 16000, sizeBytes: 32044, processingStage: 'ready',
          blob: new Blob([header.buffer, pcm], { type: 'audio/wav' }), notes: 'Original note ' + i,
          transcript: '[00:00] ' + name + ': Prototype discussion ' + i, summary: 'Prototype decision ' + i,
          meeting: { executive_summary: 'Prototype decision ' + i, people: [{ name, role: 'Collaborator' }],
            conversations: [{ title: 'Prototype ' + i, summary: 'Prototype decision ' + i, people: [{ name }], start_ms: 0,
...
async function installApi(page) {
  await page.evaluate(() => {
    window.qa = { calls: [], failure: '', signedIn: true, uid: 'journey-user', delay: false, followFailure: false };
    const reply = data => new Response(JSON.stringify(data), { headers: { 'Content-Type': 'application/json' } });
    window.SynapAuth = {
      isSignedIn: () => qa.signedIn, session: () => ({ profile: { uid: qa.uid } }), config: () => ({}),
      authedFetch: async (url, options = {}) => {
        ...
        if (qa.delay) await new Promise(resolve => { qa.release = resolve; });
        if (qa.failure) return new Response(JSON.stringify({ error: { message: qa.failure } }), { status: 503 });
        const stored = JSON.parse(localStorage.getItem('qa-cloud-merges') || '[]');
        if (options.method === 'POST') {
          const ids = JSON.parse(options.body).recording_ids;
          const merge = { merge_id: 'merge-' + Date.now(), source_recording_ids: ids, day: requestedDay,
            started_at: new Date().toISOString(), memory: { title: 'Combined prototype review', executive_summary: 'Two prototype decisions.' },
            transcript: '[00:00] Alex: First discussion\n[01:00] Blair: Second discussion' };
          stored.push(merge); localStorage.setItem('qa-cloud-merges', JSON.stringify(stored)); return reply(merge);
        }
        if (options.method === 'DELETE') {
          localStorage.setItem('qa-cloud-merges', JSON.stringify(stored.filter(m => m.merge_id !== url.split('/').pop())));
    ...
    window.SynapBackend = { people: async () => ({ people: [] }), followUps: async () => ({ follow_ups: [
      { id: 'follow-1', task: 'Review the prototype', owner: { type: 'self', display_name: 'You' }, source: { recording_id: 'journey-0', start_ms: 0 } }
    ] }), resolveFollowUp: async () => { if (qa.followFailure) throw new Error('Could not save follow-up. Please retry.'); return {}; } };
  });
  await page.evaluate(async () => {
    SynapCloudHistory.refreshUiInPlace({ source: 'journey-fixture' });
    await SynapMemoryTools.refresh(); await SynapProvenance.refresh(); await SynapBrainUI.refresh();
  });
}
...
async function snapshot(page) {
  return page.evaluate(async () => {
    const db = await new Promise(resolve => { const q = indexedDB.open('dk-pendant-recordings'); q.onsuccess = () => resolve(q.result); });
    const records = await new Promise(resolve => { const q = db.transaction('recordings').objectStore('recordings').getAll(); q.onsuccess = () => resolve(q.result); });
    db.close();
    return records.map(r => ({ id: r.id, name: r.name, notes: r.notes, summary: r.summary, transcript: r.transcript, bytes: r.blob?.size }));
  });
}
...
async function run() {
  ...
  const browser = await chromium.launch({ headless: true, ...(process.env.SYNAP_CHROMIUM_PATH ? {
    executablePath: process.env.SYNAP_CHROMIUM_PATH, args: ['--no-sandbox', '--no-zygote', '--disable-dev-shm-usage']
  } : {}) });
  try {
    for (const [mode, width] of [['light', 320], ['dark', 390], ['light', 1440]]) {
      const context = await browser.newContext({ viewport: { width, height: 900 }, reducedMotion: 'reduce', timezoneId: 'Asia/Kolkata' });
      ...
      await page.waitForTimeout(600); await seed(page); await installApi(page);
      await page.waitForFunction(() => document.querySelectorAll('#insightsList .insight-card[data-recording-id]').length === 6);
      await page.waitForTimeout(400);
      const before = await snapshot(page); assert.equal(before.length, 6);
      assert(await page.locator('#insights .section-heading').evaluate(node => {
      ...
      assert(await page.locator('.synap-merge-toolbar').isVisible(), 'Merge opens its collapsed tile');
      const checks = page.locator('.synap-merge-check'); assert.equal(await checks.count(), 6);
      await page.evaluate(() => {
        window.qaMutations = 0; window.qaObserver = new MutationObserver(changes => qaMutations += changes.length);
        qaObserver.observe(document.querySelector('#insightsList'), { childList: true, subtree: true });
      });
      await page.waitForTimeout(300);
      assert.equal(await page.evaluate(() => { qaObserver.disconnect(); return qaMutations; }), 0, 'selection settles without a DOM feedback loop');
      await checks.nth(0).click(); await checks.nth(2).click();
      assert(await page.locator('#synapMergeConfirm').isDisabled(), 'non-consecutive selection blocked');
      await checks.nth(2).click(); await checks.nth(1).click();
      ...
      assert.equal(await page.locator('.synap-merge-check[aria-pressed=true]').count(), 5, 'cannot select a sixth source');
      for (const i of [2,3,4]) await checks.nth(i).click();
      await page.evaluate(() => { qa.failure = 'Merge service unavailable. Please retry.'; });
      await page.locator('#synapMergeConfirm').click();
      await page.waitForFunction(() => document.querySelector('#synapMergeError').textContent.includes('Merge service unavailable'));
      await page.waitForTimeout(150);
      assert(await page.locator('#synapMergeError').isVisible(), 'failure remains visible');
      assert.equal(await page.locator('.synap-merge-check[aria-pressed=true]').count(), 2, 'failure retains selection');
      await page.evaluate(() => { qa.failure = ''; });
      await page.locator('#synapMergeConfirm').click();
      await page.waitForFunction(() => document.querySelectorAll('.synap-merged-card').length === 1);
      ...
      assert.deepEqual(await page.evaluate(() => JSON.parse(qa.calls.filter(c => c.method === 'POST').at(-1).body).recording_ids), ['journey-1', 'journey-0']);
      ...
      if (process.env.SYNAP_WORKFLOW_OUTPUT) {
        ...
        await page.locator('#insights').screenshot({ path: path.join(process.env.SYNAP_WORKFLOW_OUTPUT, `merge-${mode}-${width}.png`) });
      }
      await merged.getByRole('tab', { name: 'Transcript', exact: true }).click();
      assert.match(await merged.locator('.synap-provenance-transcript').innerText(), /First discussion/);
      await page.evaluate(() => SynapCloudHistory.refreshUiInPlace({ source: 'background-journey' }));
      await page.waitForTimeout(300);
      assert(await merged.evaluate(n => n.open), 'background refresh preserves merged expansion');
      assert.equal(await merged.getByRole('tab', { name: 'Transcript', exact: true }).getAttribute('aria-selected'), 'true');
      ...
      await page.reload(); await page.waitForFunction(() => window.SynapMemoryTools); await page.waitForTimeout(500); await installApi(page);
      await page.locator('.brain-tabs a[href="#insights"]').click();
      await page.waitForFunction(() => document.querySelectorAll('.synap-merged-card').length === 1);
      await page.locator('.synap-merged-card > summary').click();
      await page.evaluate(() => { qa.failure = 'Unmerge failed. Please retry.'; });
      await page.locator('.synap-unmerge').click();
      await page.waitForFunction(() => document.querySelector('#synapMergeError').textContent.includes('Unmerge failed'));
      assert.equal(await page.locator('.synap-merged-card').count(), 1);
      await page.evaluate(() => { qa.failure = ''; });
      await page.locator('.synap-unmerge').click();
      await page.waitForFunction(() => document.querySelectorAll('.synap-merged-card').length === 0);
      assert.equal(await page.locator('.synap-memory-source-hidden').count(), 0);
      assert.deepEqual(await snapshot(page), before, 'unmerge restores source views with original data');
      ...
      await page.evaluate(async () => { qa.signedIn = false; await SynapInteractionSurfaces.refresh(); SynapCompactLayout.reveal('peopleMemory'); });
      ...
      await page.evaluate(async () => { qa.signedIn = true; await SynapInteractionSurfaces.refresh(true); SynapCompactLayout.reveal('followupInbox'); qa.followFailure = true; });
      await page.locator('.synap-follow-done').click();
      await page.waitForFunction(() => document.querySelector('#followupError')?.textContent.includes('Could not save follow-up'));
      assert(await page.locator('#followupError').isVisible());
      await page.evaluate(() => { qa.followFailure = false; });
      await page.locator('.synap-follow-done').click();
      await page.waitForFunction(() => document.querySelectorAll('.synap-follow-done').length === 0);
      ...
      await page.locator('#synapMergeMemories').click(); await checks.nth(0).click(); await checks.nth(1).click();
      await page.evaluate(() => { qa.delay = true; }); await page.locator('#synapMergeConfirm').click();
      await page.waitForFunction(() => qa.release);
      await page.locator('[data-day-step="-1"]').click();
      await page.evaluate(() => { qa.delay = false; qa.release(); });
      await page.waitForTimeout(300);
      ...
      if (process.env.SYNAP_WORKFLOW_OUTPUT) {
        fs.mkdirSync(process.env.SYNAP_WORKFLOW_OUTPUT, { recursive: true });
        await page.screenshot({ path: path.join(process.env.SYNAP_WORKFLOW_OUTPUT, `${mode}-${width}.png`), fullPage: true });
      }
      await context.close();
    }
