...
(function (root) {
  ...
  let active = false,
    ...
    htmlOverflow = '';
  let selected = 'device';
  const sections = ['device', 'memory', 'appearance', 'support'],
    positions = new Map();
  // These fields save on form submission. Theme, provider, notifications and reconnect
  // have their own immediate persistence and must not create a pending draft.
  const draftFields = [
    'wakeLockInput',
    'autoProcessInput',
    'endpointInput',
  ...
  function updateSave() {
    ...
    const status = document.getElementById('settingsDraftStatus');
    if (status)
  ...
  function discardDraft() {
    ...
    JSON.parse(savedDraft).forEach((value, index) => {
      const field = document.getElementById(draftFields[index]);
      if (!field) return;
  ...
  function select(section, { focus = false } = {}) {
    ...
    for (const key of sections) {
      const tab = document.getElementById('settingsTab-' + key),
        panel = dialog.querySelector('[data-settings-panel="' + key + '"]');
      if (tab) {
        tab.setAttribute('aria-selected', String(key === selected));
        tab.tabIndex = key === selected ? 0 : -1;
      }
  ...
  function layout() {
    ...
    const keyboardInset = viewport
      ? Math.max(0, root.innerHeight - viewport.height - viewport.offsetTop)
      : 0;
    const bottom = Math.max(
      keyboardInset,
      navigation && navigation.top > root.innerHeight / 2
        ? root.innerHeight - navigation.top + 8
        : 12,
    );
    ...
    style.setProperty(
      '--settings-right',
      Math.max(0, document.documentElement.getBoundingClientRect().right - bounds.right) + 'px',
    );
  }
  function schedule() {
  ...
  function sync() {
    ...
    if (active) {
      ...
      style.setProperty('--settings-header-left', bounds.left + 'px');
      style.setProperty('--settings-header-width', bounds.width + 'px');
      headerSpacer.style.height = bounds.height + 'px';
      headerSpacer.hidden = false;
  ...
  function init() {
    ...
    nav = document.querySelector('.brain-tabs');
    if (!dialog || !header || !main) {
      dialog = null;
      return false;
    }
    ...
    dialog.querySelector('.settings-tabs')?.addEventListener('click', (event) => {
      ...
      if (tab) select(tab.id.replace('settingsTab-', ''));
    });
    ...
    for (const event of ['input', 'change'])
      dialog.addEventListener(event, () => queueMicrotask(updateSave));
  ...
  root.SynapSettingsPanel = Object.freeze({ open, close, toggle, select, saved });
