...
(function (root) {
  ...
  function decisionEntries(r) {
    ...
    for (const c of conversationsOf(r))
      for (const x of c.decisions || []) all.push({ value: x, conversation: c });
    return dedupe(all, (x) => {
      const t = textOf(x.value).toLowerCase(),
        start = sourceOffset(x.value, x.conversation);
      return t ? t + '|' + start : '';
    });
  }
  function actionEntries(r) {
    ...
    for (const c of conversationsOf(r))
      for (const x of c.action_items || []) all.push({ value: x, conversation: c });
    return dedupe(all, (x) => {
      const a = x.value || {},
        task = String(a.task || '')
          .trim()
          .toLowerCase(),
        start = sourceOffset(a, x.conversation);
      return task
        ? task +
            '|' +
            String(a.owner || '')
  ...
  function followUpEntries(r) {
    ...
    for (const c of conversationsOf(r))
      for (const x of c.follow_ups || []) all.push({ value: x, conversation: c });
    return dedupe(all, (x) => {
      const t = textOf(x.value).toLowerCase(),
        owner = String(x.value?.owner || '')
          .trim()
          .toLowerCase(),
        start = sourceOffset(x.value, x.conversation);
  ...
  function participantNames(c) {
    return dedupe(
      (Array.isArray(c.participants) ? c.participants : [])
        ...
        .map((x) => (mine(x) ? 'You' : x.trim())),
      (x) => x.toLowerCase(),
    );
  }
  function conversationModel(r, c) {
    const startMs = Math.max(0, sourceOffset(c)),
      stamp = new Date(new Date(r.createdAt).getTime() + startMs);
    return {
      key: String(r.id) + '|' + startMs + '|' + String(c.title || ''),
      id: r.id,
      startMs,
      title: c.title || r.name || 'Conversation',
      time: Number.isFinite(stamp.getTime()) ? fmt(stamp) : '',
      summary: String(c.summary || ''),
      participants: participantNames(c),
      decisions: (c.decisions || []).map(textOf).filter(Boolean),
      actions: (c.action_items || [])
        .filter((a) => a?.task)
        .map((a) => ({
          text: String(a.task),
          meta: [mine(a.owner) ? 'You' : a.owner, a.due_date].filter(Boolean).join(' · '),
        })),
  ...
  function renderBrief(list) {
    ...
    if (toggle) {
      ...
      toggle.textContent = briefExpanded ? 'Show less' : 'Read more';
      toggle.setAttribute('aria-expanded', String(briefExpanded));
    }
    const sources = $('#dayBriefSources');
    if (sources) {
      sources.hidden = !briefExpanded || !summaries.length;
      sources.innerHTML = summaries
        .map(
          (r) =>
            `<button type="button" class="source-jump" data-id="${esc(r.id)}" data-offset-ms="0">${esc(r.name || 'Recording')} · ${esc(fmt(r.createdAt))}</button>`,
        )
        .join('');
    }
    const status = $('#dayBriefStatus');
    if (status) {
      status.hidden = !readError && (!pending || !summaries.length);
      status.textContent = readError
        ? 'Saved memories could not refresh. Showing the last available summaries.'
        : `${pending} recording${pending === 1 ? '' : 's'} awaiting a summary.`;
    }
    const recent = $('#dayBriefLastActive');
    if (recent) {
      // Offer a way out of an empty day using the same local date selection as the calendar.
      const latest =
        !list.length && !readError
          ? records.reduce((latest, recording) => {
              const key = day(recording.createdAt);
              return key <= todayKey && key > latest ? key : latest;
            }, '')
          : '';
      recent.hidden = !latest;
      recent.dataset.day = latest || '';
      if (latest)
        recent.textContent =
          'View ' +
          new Date(latest + 'T12:00:00').toLocaleDateString([], {
            day: 'numeric',
            month: 'short',
            ...(latest.slice(0, 4) !== today().slice(0, 4) ? { year: 'numeric' } : {}),
          }) +
          ' →';
    }
  ...
  function renderConversations(items) {
    const lane = $('#dayConversations');
    if (lane) lane.hidden = !items.length;
    const container = $('#conversationList');
    if (!container) return;
    const previous = new Map(
        [...container.querySelectorAll('.conversation-digest')].map((n) => [n.dataset.key, n]),
      ),
      keys = new Set();
    if (items.length) container.querySelector('.brain-empty')?.remove();
    items.slice(0, conversationLimit).forEach(({ r, c }, index) => {
      const item = conversationModel(r, c),
        key = item.key;
      keys.add(key);
      let node = previous.get(key);
      if (!node) {
        node = document.createElement('details');
        node.className = 'conversation-digest';
        node.dataset.key = key;
        node.open = false;
      }
      const markup = digestMarkup(item);
      if (node.__markup !== markup) {
        node.innerHTML = markup;
        node.__markup = markup;
      }
    ...
    if (!items.length)
      container.innerHTML =
        '<p class="brain-empty">Conversation summaries will appear here after processing.</p>';
    const count = $('#conversationPageCount');
    if (count)
      count.textContent = items.length
        ? `${Math.min(conversationLimit, items.length)} of ${items.length} summaries`
        : '';
    const more = $('#showMoreConversations');
    if (more) more.hidden = items.length <= conversationLimit;
  }
  ...
  function rows(items, kind) {
    ...
    return items
      .map(
        (x) =>
          `<button type="button" class="brain-action-row source-jump" data-id="${esc(x.r.id)}" data-offset-ms="${Math.max(0, Number(x.startMs) || 0)}"><span class="brain-action-icon">${kind === 'decision' ? '✓' : kind === 'mine' ? '→' : '←'}</span><span><strong>${esc(x.text)}</strong>${x.meta ? `<small>${esc(x.meta)}</small>` : ''}</span></button>`,
      )
      .join('');
  }
    ...
    for (const r of list || []) {
      ...
      for (const entry of decisionEntries(r)) {
        ...
        decisions.push({
          ...
          meta: entry.conversation?.title || r.name || fmt(r.createdAt),
        });
      }
  ...
  function ensureSections() {
    ...
    if (insights && !$('#followupInbox')) {
      ...
      inbox.id = 'followupInbox';
      inbox.className = 'section-card followup-inbox';
      inbox.innerHTML =
        '<div class="section-heading"><div><p class="section-eyebrow">FOLLOW THROUGH</p><h2>Follow-up inbox <span id="followupCount" class="count-badge">0</span></h2><p class="section-copy">What you owe, what others owe you, and what still needs closure.</p></div></div><div class="followup-tabs"><button data-follow="mine" class="active">You owe</button><button data-follow="waiting">Waiting on</button><button data-follow="all">All</button></div><div id="followupList"></div>';
      insights.after(inbox);
    }
    const inbox = $('#followupInbox');
    if (inbox && !$('#peopleMemory')) {
      const people = document.createElement('section');
      people.id = 'peopleMemory';
      people.className = 'section-card people-memory';
      people.innerHTML =
        '<div class="section-heading"><div><p class="section-eyebrow">PEOPLE MEMORY</p><h2>People</h2><p class="section-copy">Who you spoke with, what you discussed, and what remains open.</p></div></div><div id="peopleList" class="people-grid"></div>';
      inbox.after(people);
    }
    ...
    if (nav && !nav.querySelector('a[href="#myActions"]')) {
      const a = document.createElement('a');
      a.href = '#myActions';
      a.innerHTML =
        '<svg aria-hidden="true" viewBox="0 0 24 24"><path d="m3 6 2 2 4-4M12 6h9M3 12l2 2 4-4M12 12h9M3 18l2 2 4-4M12 18h9"/></svg><span>Actions</span>';
  ...
  function install() {
    ...
    $('#dayBriefLastActive')?.addEventListener('click', (event) => {
      const key = event.currentTarget.dataset.day,
        picker = $('#datePicker');
      if (!key || !picker) return;
      picker.value = key;
      picker.dispatchEvent(new Event('change', { bubbles: true }));
      // The empty-state link disappears after navigation; leave focus on the new day.
    ...
    $$('.followup-tabs button').forEach((b) =>
      b.addEventListener('click', () => {
        $$('.followup-tabs button').forEach((x) => x.classList.toggle('active', x === b));
        renderFollowups(b.dataset.follow);
      }),
    );
    document.addEventListener('click', (e) => {
      const b = e.target.closest('.source-jump');
      if (b) jump(b.dataset.id, Number(b.dataset.offsetMs) || 0);
      const p = e.target.closest('.person-card');
      if (p) {
        root.SynapAsk?.open
          ? root.SynapAsk.open(p.dataset.person)
          : (() => {
              location.hash = '#ask';
              $('#askInput').value = p.dataset.person;
              answer(p.dataset.person);
            })();
      }
    });
    ...
    ].forEach((name) => root.addEventListener(name, scheduleRefresh));
  }
  ...
  function render() {
    ...
    const list = records
        .filter((r) => day(r.createdAt) === selected())
    ...
    if (d.conversations.length)
      bits.push(`${d.conversations.length} conversation${d.conversations.length === 1 ? '' : 's'}`);
    if (d.decisions.length)
      bits.push(`${d.decisions.length} decision${d.decisions.length === 1 ? '' : 's'}`);
    if (d.my.length) bits.push(`${d.my.length} commitment${d.my.length === 1 ? '' : 's'}`);
    if ($('#pulseLine'))
      $('#pulseLine').textContent = bits.length
        ? bits.join(' · ')
        : 'Your decisions and next steps, together.';
    const chips = [
      ...d.people.slice(0, 4).map((x) => ['person', x.name, x.role]),
    ...
    if ($('#contextChips')) {
      $('#contextChips').innerHTML = chips
        .map(
          ([k, v, role]) =>
            `<button type="button" data-query="${esc(v)}"><span>${k === 'person' ? '@' : '#'}</span>${esc(v)}${role && role !== 'unknown' ? `<small>${esc(role)}</small>` : ''}</button>`,
        )
        .join('');
      $$('#contextChips button').forEach((b) =>
        b.addEventListener('click', () => {
          root.SynapAsk?.open
            ? root.SynapAsk.open(b.dataset.query)
            : (() => {
                location.hash = '#ask';
    ...
    if ($('#peopleList') && !root.SynapInteractionSurfaces)
      $('#peopleList').innerHTML = d.people.length
        ? d.people
            .slice(0, 8)
            .map((p) => {
              const top = [...p.topics]
                  ...
                  .map((x) => x[0])
                  .join(' · '),
                open = d.waiting.filter(
                  (x) => String(x.owner).toLowerCase() === p.name.toLowerCase(),
                ).length;
              return `<button type="button" class="person-card" data-person="${esc(p.name)}"><span class="person-avatar">${esc(p.name.charAt(0).toUpperCase())}</span><span><strong>${esc(p.name)}</strong><small>${esc(p.role && p.role !== 'unknown' ? p.role : top || p.count + ' memories')}</small>${open ? `<em>${open} open follow-up${open > 1 ? 's' : ''}</em>` : ''}</span></button>`;
            })
            .join('')
  ...
  function renderFollowups(mode) {
    const scoped = records.filter((r) => day(r.createdAt) === selected()),
      d = derive(scoped),
      items = mode === 'mine' ? d.my : mode === 'waiting' ? d.waiting : [...d.my, ...d.waiting];
    if ($('#followupCount')) $('#followupCount').textContent = d.my.length + d.waiting.length;
    if ($('#followupList'))
      $('#followupList').innerHTML = items.length
        ? rows(items, mode === 'mine' ? 'mine' : 'waiting')
        : '<p class="brain-empty">Nothing open right now.</p>';
  }
  function hay(r) {
    const m = meeting(r),
      conv = conversationsOf(r).flatMap((c) => [
        c.title,
        c.summary,
      ...
      people = (m.people || []).flatMap((p) => [p.name, p.role, p.evidence]),
      actions = actionEntries(r).map(({ value: a }) => [a.task, a.owner, a.due_date].join(' ')),
      decisions = decisionEntries(r).map((x) => textOf(x.value)),
      followups = followUpEntries(r).map((x) => textOf(x.value));
    return [
      r.name,
      r.summary,
      r.transcript,
  ...
  function answer(q) {
    ...
    if (!out) return;
    if (!q) {
      out.innerHTML = '<p class="brain-empty">Ask a question about your memories.</p>';
      return;
    }
    ...
    for (const { r } of ranked) {
      ...
      if (/who|person|people|spoke|talk/.test(lower)) {
        ...
        actionEntries(r)
          .filter((x) => mine(x.value?.owner) || !x.value?.owner)
          .forEach((x) =>
            facts.push({
              r,
              text: x.value.task + (x.value.due_date ? ' · ' + x.value.due_date : ''),
              startMs: sourceOffset(x.value, x.conversation),
            }),
          );
      } else if (/waiting|follow|pending/.test(lower)) {
        actionEntries(r)
          .filter((x) => x.value?.owner && !mine(x.value.owner))
          .forEach((x) =>
            facts.push({
              r,
              text: x.value.task + ' — ' + x.value.owner,
              startMs: sourceOffset(x.value, x.conversation),
            }),
          );
        followUpEntries(r).forEach((x) => {
          const t = textOf(x.value),
            owner = String(x.value?.owner || '').trim();
          if (t)
            facts.push({
              r,
              text: t + (owner && !mine(owner) ? ' — ' + owner : ''),
              startMs: sourceOffset(x.value, x.conversation),
            });
        });
      } else {
        const cs = conversationsOf(r).filter((c) =>
          terms.some((t) =>
            [c.title, c.summary, ...(c.people || []).map((p) => p.name), ...(c.topics || [])]
              .join(' ')
              .toLowerCase()
              .includes(t),
          ),
        ...
        if (cs.length)
          cs.forEach((c) =>
            facts.push({
              r,
              text: (c.title ? c.title + ': ' : '') + c.summary,
              startMs: sourceOffset(c),
            }),
          );
        else {
          const t = m.executive_summary || r.summary || r.transcript;
          if (t) facts.push({ r, text: String(t).replace(/\s+/g, ' ').slice(0, 280), startMs: 0 });
        }
      }
    }
    if (!facts.length) {
      out.innerHTML =
        '<p class="brain-empty">I could not find a grounded answer in your processed memories.</p>';
      return;
    }
    out.innerHTML = `<div class="answer-copy"><span class="answer-mark">✦</span><div><strong>From your memory</strong>${facts
      ...
      .join('')}</div></div><div class="answer-sources"><span>Sources</span>${facts
      .slice(0, 6)
      .map(
        (f) =>
          `<button type="button" class="source-jump" data-id="${esc(f.r.id)}" data-offset-ms="${Math.max(0, Number(f.startMs) || 0)}">${esc(f.r.name || 'Conversation')} · ${esc(new Date(f.r.createdAt).toLocaleDateString([], { day: 'numeric', month: 'short' }))}</button>`,
      )
      .join('')}</div>`;
  }
  function jump(id, offsetMs = 0) {
    if (root.SynapProvenance?.openSource) return root.SynapProvenance.openSource(id, offsetMs);
    const r = records.find((x) => String(x.id) === String(id));
    if (!r) return;
    const p = $('#datePicker');
    if (p) {
      p.value = day(r.createdAt);
