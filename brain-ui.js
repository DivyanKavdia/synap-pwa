...
(function(root){
  ...
  function rows(items,kind){if(!items.length)return'<p class="brain-empty">Nothing detected yet.</p>';return items.map(x=>`<button type="button" class="brain-action-row source-jump" data-id="${esc(x.r.id)}" data-offset-ms="${Math.max(0,Number(x.startMs)||0)}"><span class="brain-action-icon">${kind==='decision'?'✓':kind==='mine'?'→':'←'}</span><span><strong>${esc(x.text)}</strong>${x.meta?`<small>${esc(x.meta)}</small>`:''}</span></button>`).join('')}
  ...
  root.SynapBrainUI={__singleton:true,derive,refresh,answerLocal:answer,decisionEntries,actionEntries,followUpEntries,sourceOffset,summaryOf,conversationModel};
