...
let session=null,guardBound=false;
...
function pendantBusy(){const state=String(document.body?.dataset?.state||'');return['starting','recording','stopping','saving','updating'].includes(state)}
function bindMutualExclusion(){if(guardBound)return;guardBound=true;document.addEventListener('click',event=>{if(!session)return;const start=event.target.closest?.('#startButton');if(!start)return;event.preventDefault();event.stopImmediatePropagation();setStatus('Stop the online meeting capture before starting the pendant.')},true)}
async function start(options={}){
  if(session)throw new Error('Desktop capture is already running.');
  if(pendantBusy())throw new Error('Finish the pendant recording before starting an online meeting capture.');
