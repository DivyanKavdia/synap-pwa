...
function stop(reason='desktop-capture'){
  ...
  if(!s.stopping)s.stoppedAt=Date.now();
...
function state(){return session?{active:true,phase:session.phase,recordingId:session.recordingId,startedAt:session.startedAt,stoppedAt:session.stoppedAt}:{active:false,phase:'idle',recordingId:null}}
