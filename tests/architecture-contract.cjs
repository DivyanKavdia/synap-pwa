...
assert(enhancements.includes("SHELL_REVISION='1.0.0-shell150-hey-snap-ble-gate'"),'update-notice generation must match the service worker');
...
assert(sw.includes("const CACHE_REVISION='1.0.0-shell150-hey-snap-ble-gate';"),'service worker revision must advance with the architecture graph');
