...
assert(enhancements.includes("SHELL_REVISION='1.0.0-shell52-speech-safe'"),'update-notice generation must match the service worker');
...
assert(sw.includes("const CACHE_REVISION='1.0.0-shell52-speech-safe';"),'service worker revision must advance with the architecture graph');
