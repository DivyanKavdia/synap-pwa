...
assert(enhancements.includes("SHELL_REVISION='1.0.0-shell89-pcm-integrity'"),'update-notice generation must match the service worker');
...
assert(sw.includes("const CACHE_REVISION='1.0.0-shell89-pcm-integrity';"),'service worker revision must advance with the architecture graph');
