...
assert(enhancements.includes("SHELL_REVISION='1.0.0-shell40-weekly'"),'update-notice generation must match the service worker');
...
assert(sw.includes("const CACHE_REVISION='1.0.0-shell40-weekly';"),'service worker revision must advance with the architecture graph');
