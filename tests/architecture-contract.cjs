...
const core=[
  ...
  'ota.js','releases.js','moments.js','app.js','google-auth.js','synap-backend.js','processing-pipeline-ui.js','memory-ui-fix.js','cloud-history.js',
...
assert(enhancements.includes("SHELL_REVISION='1.0.0-shell58-connection-resume'"),'update-notice generation must match the service worker');
...
assert(sw.includes("const CACHE_REVISION='1.0.0-shell58-connection-resume';"),'service worker revision must advance with the architecture graph');
