...
assert.match(config, /memoryModel:[\s\S]*gemini-3\.8-flash['"]/);
...
assert.match(memory, /thinking_level: 'high'[\s\S]{0,120}usage_label: 'memory_extract'/);
...
assert.match(deploy, /memory_model="\$\{SYNAP_GEMINI_MEMORY_MODEL:-gemini-3\.8-flash\}"/);
