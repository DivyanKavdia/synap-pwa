...
async function workerTests(){
  ...
  assert(installed.includes('./runtime-compat.js'));assert(installed.includes('./processing-recovery.js'));assert(installed.includes('./dashboard-ui.js'));assert(installed.includes('./ask-synap.js'));assert(installed.includes('./sleep-state-guard.js'));
  ...
  assert.equal(reply.type,'APP_VERSION');assert.equal(reply.version,'1.0.0');assert.equal(reply.release,'1.0.0');assert.equal(reply.revision,'1.0.0-audio2');assert.equal(reply.shellRevision,'1.0.0-shell33-power');
}
