...
function context(devices=[]){
  ...
  const c={gattServer:null,controlCharacteristic:null,audioCharacteristic:null,firmwareBusy:false,console,Boolean,Number,String,Date,Error,Promise,WeakSet,AbortController,withTimeout:p=>p,
    ...
    localStorage:{getItem:k=>saved.get(k)??null,setItem:(k,v)=>saved.set(k,v)},connectionEpoch:0,bluetoothDevice:null,connectInProgress:false,manualDisconnect:false,reconnectSelectionRequired:false,rapidNativeLinkFailures:0,finalizing:false,currentRecordingId:null,recordingReconnectPending:false,reloadRecoveryRunning:false,lastReloadRecoveryAt:0,reconnectTimer:null,reconnectAttempts:0,MAX_AUTO_RECONNECT_ATTEMPTS:3,clearTimeout(){},
...
async function workerTests(){
  ...
  assert.equal(reply.type,'APP_VERSION');assert.equal(reply.version,'1.0.0');assert.equal(reply.release,'1.0.0');assert.equal(reply.revision,'1.0.0-audio6');assert.equal(reply.shellRevision,'1.0.0-shell165-unified-hey-snap');
}
...
(async()=>{
  ...
  assert(native.calls.includes('reselect-required'),'nativeReason 2 must stop automatic Bluefy retries after the second rapid failure');
  assert.equal(native.c.reconnectSelectionRequired,true,'stale Bluefy handle must require explicit reselection');
  assert(!native.calls.includes('refresh-connect'),'stale permitted handle must not be recycled through getDevices');
  await workerTests();
