...
test('a newer shell offers an update even when the Bluetooth protocol revision is unchanged',async()=>{
  ...
  const c={APP_REVISION:'1.0.0-audio5',APP_SHELL_REVISION:'loaded-shell',Date,
  ...
  const send=shellRevision=>handlers.message({data:{type:'APP_VERSION',revision:'1.0.0-audio5',shellRevision}});
