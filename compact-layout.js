...
(function (root) {
  ...
  function setExpanded(tile, expanded, save = false) {
    tile.body.hidden = !expanded;
    tile.section.classList.toggle('is-expanded', expanded);
    ...
    if (!expanded) {
      if (tile.body.contains(document.activeElement)) tile.button.focus({ preventScroll: true });
      if (tile.section.id === 'library')
        tile.body.querySelectorAll('audio').forEach((audio) => audio.pause());
    }
    if (save) {
      preferences[tile.section.id] = expanded;
      persist();
    }
  }
  function register(section, heading, title, defaultOpen) {
    if (!section || !heading || tiles.has(section.id)) return;
    section.classList.add('workspace-tile');
    ...
    const connect = heading.querySelector('#connectButton');
    if (connect) body.appendChild(connect);
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'tile-toggle';
    button.setAttribute('aria-controls', body.id);
    button.innerHTML = '<svg aria-hidden="true"><use href="#i-chevron"/></svg>';
    heading.appendChild(button);
    const tile = { section, heading, body, button, title };
    tiles.set(section.id, tile);
    const adopt = () => {
      for (const child of [...section.childNodes])
        if (child !== heading && child !== body) body.appendChild(child);
    };
    adopt();
    setExpanded(
      tile,
      typeof preferences[section.id] === 'boolean' ? preferences[section.id] : defaultOpen,
    );
    button.addEventListener('click', () => setExpanded(tile, body.hidden, true));
    heading.addEventListener('click', (event) => {
      if (!event.target.closest('button,a,input,select,textarea,label,summary')) button.click();
    });
  ...
  function scan() {
    for (const [id, selector, title, open] of configs) {
      const section = document.getElementById(id);
      register(section, section?.querySelector(selector), title, open);
    }
  }
  function reveal(target) {
    ...
    for (const tile of tiles.values())
      if (tile.section === target || tile.section.contains(target)) setExpanded(tile, true);
  }
  function init() {
    scan();
    const main = document.querySelector('main'),
      brief = document.querySelector('.day-brief');
